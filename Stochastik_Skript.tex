...
\usepackage{ulem}
%\usepackage{emph}
\author{Maik Pickl und Fred Brockstedt \\ HU-Berlin.de}
...
\begin{center}
  ...
  $\sigma(X)=\sqrt{Var(X)}$ heißt Streuung (Standardabweichung) \index{Streuung}\index{Standardabweichung}
\end{center} 
...
\begin{example}
  ...
  \begin{eqnarray*}
    ...
    &=& \sum\limits_{k=0}^nk^2\binom{n}{k}p^k(1-p)^{n-k}\\
...
\begin{defn} \index{stochastische Konvergenz} \index{p-fast-sicher} \index{Konvergenz im $p$-ten Mittel} % flashcard-name: Konvergenz: stoachstische, p-fast-sicher p-ten Mittel
  ...
  \item $(X_i)_{i \in \mathbb{N}}$ konvergiert stochastisch (in
    Wahrscheinlichkeit) gegen eine Zufallsvariable $X$ falls $\forall\varepsilon >0$ gilt:
    \begin{center}
...
\begin{example}[Random Walk] \index{Random Walk}
  Betrachte ein Münzwurf-Modell mit Parameter $p=\frac{1}{2}$. Seien
...
\begin{defn}  % flashcard-name: gleichmäßig integrierbar
  ...
  heißt \index{gleichmäßig integrierbar} gleichmäßig integrierbar (GI) falls gilt
  \begin{center}
...
\begin{defn} \index{Dichtefunktion}  % flashcard-name: absolut stetig
  $F$ bzw. $\mu$ heißen \emph{absolut stetig} wenn es eine
  messbare Funktion $f\geq 0$ (genannt \emph{Dichtefunktion})
  gibt, so dass gilt:
  \begin{center}
...
\begin{defn}  % flashcard-name: Was bedeutet $\mu$-randlos
  Eine Menge $A \in B(S)$ heißt \index{$\mu$-randlos} $\mu$-randlos falls
  \begin{center}
    $\mu(\partial A)=0$ 
  \end{center}
...
\begin{proof} \ 
  ...
  Eine Folge $(\mu_n)_{n \in \N}$ von Wahrscheinlichkeitsmaßen konvergiert vag gegen
  $\mu$, falls
  \begin{center}
...
% \textbf{Korollar 10.5}\\\\
\begin{cor}
  Seien $\mu_n,\mu$ Wahrscheinlichkeitsmaße auf $(\Sigma,\mathcal{A})$ mit
...
\uline{Ziel:} $\mu,\nu$ Wahrscheinlichkeitsmaße auf $(\Omega,\mathcal{A})$ und
...
\begin{defn} \index{Dynkin-System} % flashcard-name: Was muss gelten für ein Dynkinsystem 
...
% \textbf{Beispiel 11.2}
\begin{example}
  Seien $\mu_{1},\mu_{2}$Wahrscheinlichkeitsmaße auf $(\Omega,\mathcal{A}).$Dann ist
  \[
...
% \textbf{Korollar 11.5}\\
\begin{prop}
  Seien $\mu_{1},\mu_{2}$ Wahrscheinlichkeitsmaße auf $(\Omega,\mathcal{A})$ und
...
\begin{example}
  ...
  monotoner Vektorraum \index{monotoner Vektorraum} (MVR) falls mit $f_{n}\in\mathcal{H}$ und
  $f_{n}\nearrow f$
...
\begin{prop}[über monotone Klassen]
  ...
  meßbar sind)\\
  Dann ist jede $\sigma(\mathcal{A})$ messbare Funktion in
  $\mathcal{A}$
  enthalten.
\end{prop}
...
Dann ist jede $\sigma(C_{b}(S))$-messbare Funktion in $\mathcal{H}$
enthalten. Z.z.
\[
...
\begin{rem}
  ...
  \index{terminale sigma} $\sigma$-Algebra (,,tail field``) ist gegeben durch
  \[
  ...
  \item $\limsup_{i\to\infty}\left\{ X_{i}=1\right\} $ ($+1$ kommt unendlich oft vor) 
  \end{itemize}
\end{rem}
\begin{prop}[0-1-Gesetz von Kolmogorov] \index{0-1 Gesetz von Kolmogorov}
  \begin{comment}
    Eigentlich Satz 1.6
  \end{comment}
...
\begin{proof}
  ...
  \]
  unabhängig (,,weniger Ereignisse zu testen``). Also sind
  \[
...
Dann gilt
\[
\lim_{n\to\infty}\frac{1}{n}\sum_{i=1}^{n}\left(X_{i}-\mathbb{E}\left[X_{i}\right]\right)=0\ P\mbox{-f.s}.
\]
...
\begin{defn}  % flashcard-name: Wie ist Unabhängigkeit definiert?
  Eine Familie $\left(X_{i}\right)_{i\in I}$ von Zufallvariablen auf
  $\left(\Omega,\mathcal{A},\mathbb{P}\right)$ heißt
  \index{unabhängig} unabhängig, falls
...
\begin{prop}[Kolmogorov] \index{Kolmogorov} % flashcard-name: Für $(X_i)_{i\in\mathbb{N}}\in\mathbb{L}^1$. Dann gilt?
  ...
  \begin{equation}
    \frac{1}{n}\sum_{i=1}^{n}X_{i}\to m\ P\mbox{-f.s}.\label{eq:kolmogorov}
  \end{equation}
...
\begin{prop}[Etemadi] \index{Etemadi}
  ...
  \[
  \frac{1}{n}\sum_{i=1}^{n}X_{i}\to m\ P\mbox{-f.s}.
  \]
\end{prop}
\begin{proof}[Etemadi] \index{Etemadi}
  ...
  \item [{(A)}] Wir zeigen zunächst: wir können die $X_{i}$ abschneiden.
    ...
    Wir wollen zeigen:
    \begin{equation}
      P\left[\lim_{n\to\infty}\frac{1}{n}\left(S_{n}-\tilde{S}_{n}\right)=0\right]=1\label{eq:etamadi}
    \end{equation}
  ...
  \item [{(B)}] \marginpar{Vorlesung vom 5.6.2013}Wir wissen ,,Schnelle
    ...
    \begin{equation}
      \frac{\tilde{S}_{k_{n}}-\mathbb{E}\tilde{S}_{k_{n}}}{k_{n}}\to0\ P\mbox{-f.s}.\label{eq:etamadi(B)}
    \end{equation}
    wobei
    \[
    ...
    \]
    Damit folgt dann mit \ref{eq:etamadi(B)}
    \[
    \lim_{n\to\infty}\frac{1}{k_{n}}\tilde{S}_{k_{n}}=m\ P\mbox{-f.s}.
    \]
    D.h. es existiert eine Nullmenge $N_{\alpha}$ mit 
    \[
    \frac{1}{k_{n}}\tilde{S}_{k_{n}}\left(\omega\right)\to m\ \forall\omega\notin N_{\alpha}
    \]
    Wir befreien uns nun von der Teilfolge $\left(k_{n}\right)$. Sei $l\in\mathbb{N}$.
    ...
    \]
    Wählen nun $\alpha_{n}\searrow1$ mit entsprechender Nullmenge $N_{\alpha_{n}}$
    und setze
    \[
  ...
  \item [{(C)}] Wir zeigen nun schnelle stochastische Konvergenz entlang
    $\left(k_{n}\right)$. Dazu reicht es zu zeigen (Korollar I 1.10)
    \[
    \sum_{n\geq1}P\left[\left|\frac{\tilde{S}_{k_{n}}-\mathbb{E}\left[\tilde{S}_{k_{n}}\right]}{k_{n}}\right|\geq\varepsilon\right]<\infty
    \]
    Dann folgt \ref{eq:etamadi(B)}. Aus ,,paarweise unabhängig`` folgt
    ,,paarweise unkorreliert``. Also mit Cebysev: 
    \begin{eqnarray*}
    ...
    \begin{eqnarray}
      ...
      & \overset{(*)}{\leq} & \sum_{i=1}^{\infty}\frac{c}{i^{2}}\mathbb{E}\left[\tilde{X}_{i}^{2}\right]\label{eq:etamadi(C)}\\
  ...
  \item [{(D)}] Es bleibt \ref{eq:etamadi(C)} ({*}) zu zeigen
    \[
    \sum_{n:k_{n}\geq i}\frac{1}{k_{n}^{2}}\leq\frac{c}{i^{2}}
    \]
    Es gilt
    ...
    \begin{equation}
      k_{n_{i}}=\llcorner\alpha^{n_{i}}\lrcorner\geq i\ \left(\alpha^{n_{i}}\geq i\right)\label{eq:etamadi(D)}
    \end{equation}
    Damit gilt
    \[
    ...
    \begin{eqnarray*}
      ...
      & \overset{\ref{eq:etamadi(D)}}{\leq} & \frac{c_{\alpha}^{-2}}{1-\alpha^{-2}}\frac{1}{i^{2}}\\
      & = & \frac{c}{i^{2}}
    \end{eqnarray*}
...
\begin{example}
  ...
  und
  \begin{eqnarray*}
    X_{n}\left(\omega\right) & \leq &
    e^{n\left(\alpha+\varepsilon\right)}\mbox{ für }n\geq N\left(\omega\right)\\
...
Damit 
\[
X_{n}\left(\omega\right)\to0\ P\mbox{-f.s}.
\]
...
\]
mit unbekannten $m\in\mathbb{R}$ und $\sigma^{2}>0$. Nach Kolmogorov:
\[
m_{n}:=\frac{1}{n}\sum_{i=1}^{n}X_{i}\left(\omega\right)\to m\ \mathbb{P}\mbox{-f.s}.
\]
Kolmogorov angewendet auf die $X_{i}^{2}$ liefert 
\[
\sigma_{n}^{2}:=\frac{1}{n}\sum\left(X_{i}^{2}-m_{n}\right)\overset{P\mbox{-f.s}.}{\longrightarrow}\sigma^{2}
\]
...
\]
Wir definieren die \index{empirische Verteilung}\emph{empirische Verteilung}
\[
...
\end{eqnarray*}
$\rho_{n}\left(\omega_{i}\right)$ ist ein zufälliges Wahrscheinlichkeitsmaß
...
\begin{prop}  % flashcard-name: P-fast alle $\omega \in \Omega$ sind im Rahmen des Gesetzes von Kolmogorov?
...
\begin{defn}   % flashcard-name: Wie ist Verteilung definiert?
  ...
  Dann heißt die Verteilung \index{Verteilung}
  \[
...
\]
Haben $\mu_{1},\mu_{2}$ Dichten $f_{1},f_{2}$ so gilt
$\mu_{1}*\mu_{2}$ hat dichte $f$ mit 
\[
...
\]
Hat eine Zufallsvariable $X$ die Verteilung $\mu$, so ist die charakteristische
...
\item punktweise Konvergenz von charakteristischen Funktionen impliziert
...
\[
\underset{\mbox{Taylorentwicklung mit Integraldarstellung für den Rest-Term}}{e^{iX}=\underbrace{\sum_{k=0}^{n}\frac{\left(ix\right)^{k}}{k!}}+\underbrace{\frac{i^{n+1}}{n!}\int_{0}^{X}\left(x-s\right)^{n}e^{is}ds}}
\]
...
\begin{example}[Charakteristische Funktion der Normalverteilung] \index{Normalverteilung}
  ...
  Verteilung. Wir substituieren $y=\frac{x^{2}}{2}$
  \[
...
\end{itemize}
Dann erhält man sofort, den \emph{Satz von DeMoivre-Laplace}:\index{Satz von DeMoivre-Laplace}
...
\begin{proof}
  ...
  \[
  I_{T}=\int_{-\infty}^{\infty}\left[\frac{sign\left(x-a\right)}{\pi}S\left(T\left|x-a\right|\right)-\frac{sign\left(x-b\right)}{\pi}S\left(T\left|x-b\right|\right)\right]\mu\left(dx\right)
  \]
...
\begin{proof}
  ...
  \begin{fact*}
    für jedes $g\in C_{0}\left(\mathbb{R}\right)$ existiert $f\in C_{0}^{\infty}$
  ...
  \item [{(d)}] Sei dazu 
    ...
    \begin{eqnarray*}
      \sum_{k=1}^{n}\left(\mathbb{E}\left[\delta\left(\frac{Y}{\sqrt{n}}\right)\frac{Y^{2}}{n}\right]+\mathbb{E}_{n}\left[\delta\left(\frac{\tilde{Y}_{n}}{\sqrt{n}}\right)\frac{\tilde{Y}_{n}^{2}}{n}\right]\right)
      & \leq &
      \mathbb{E}_{n}\left[\underset{\to0}{\underbrace{\delta\left(\frac{Y}{\sqrt{n}}\right)}}\cdot
        Y^{2}\right] \\
      & & -\int\underset{\to0}{\underbrace{\delta\left(\frac{Y}{\sqrt{n}}\right)}}\cdot Y^{2}d\mathcal{N}\left(0,Var\left(Y\right)\right)\\
...
eine Familie von Wahrscheinlichkeitsmaßen.Es sei 
\[
...
\begin{defn}
  ...
  \emph{besser} \index{besser} als $G_{n}^{*}$, falls 
  \[
...
\end{description}
\begin{defn}
  Es liege einer der obigen Fälle vor. Für jedes $x^{\left(n\right)}=\left(x_{1}\ldots x_{n}\right)\in\mathbb{R}^{n}$
  heißt 
  \[
